...
class App extends React.Component {
  ...
  componentDidMount() {
    ...
    request.onload = function() {
      _this.setState({ tickets: request.response.tickets });
    };
    request.send();
    ...
    touchReg.bind(filters, 'swipe', function(e) {
      ...
      if (direction < 210 && direction > 150) { // degrees of direction
        filters.classList.remove('showed');
      }
    });
    touchReg.bind(document.getElementById('touch-region'), 'swipe', function(e) {
      var direction = e.detail.data[0].currentDirection;
      if (direction > 330 || direction < 30) { // degrees of direction
  ...
  changeCur(e) {
    ...
    var request = new XMLHttpRequest();
    request.open('GET', `https://ratesapi.io/api/latest?base=RUB&symbols=${val}`);
    request.responseType = 'json';
    ...
    request.ontimeout = request.onerror = setDefaultRates;
  ...
  filterTransfers(e) {
    const isOnly = e.target.getAttribute('data-btn') === 'only';
...
const Filters = (props) => (
  <div id='filters-container'>
    <div id='filters'>
      ...
      <div id='transfer'>
        <div>
          <input name='transfers' data-value='all' type='checkbox' onChange={props.changeTransfers}
            checked={props.transfers.every(el => el)} />
        ...
        <div>
          <input name='transfers' data-value='0' type='checkbox' onChange={props.changeTransfers}
            checked={props.transfers[0]} />
          <div className='checkbox-layer'>
            <div className='checkbox'><img src='img/Rectangle 32.svg' /></div>
              Без пересадок
            <span data-btn='only' data-value='0' onClick={props.changeTransfers}></span>
          </div>
        </div>
        <div>
          <input name='transfers' data-value='1' type='checkbox' onChange={props.changeTransfers}
            checked={props.transfers[1]} />
          <div className='checkbox-layer'>
            <div className='checkbox'><img src='img/Rectangle 32.svg' /></div>
              1 пересадка
            <span data-btn='only' data-value='1' onClick={props.changeTransfers}></span>
          </div>
        </div>
        <div>
          <input name='transfers' data-value='2' type='checkbox' onChange={props.changeTransfers}
            checked={props.transfers[2]} />
          <div className='checkbox-layer'>
            <div className='checkbox'><img src='img/Rectangle 32.svg' /></div>
              2 пересадки
            <span data-btn='only' data-value='2' onClick={props.changeTransfers}></span>
          </div>
        </div>
        <div>
          <input name='transfers' data-value='3' type='checkbox' onChange={props.changeTransfers}
            checked={props.transfers[3]} />
          <div className='checkbox-layer'>
            <div className='checkbox'><img src='img/Rectangle 32.svg' /></div>
              3 пересадки
            <span data-btn='only' data-value='3' onClick={props.changeTransfers}></span>
          </div>
        </div>
      </div>
    </div>
